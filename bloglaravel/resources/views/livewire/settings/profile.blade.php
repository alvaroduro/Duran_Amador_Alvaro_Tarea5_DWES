...
<section class="w-full">
    ...
    <x-settings.layout :heading="__('Perfil')" :subheading="__('Actualiza tu email y nombre de usuario')">
                ...
                @if (auth()->user() instanceof \Illuminate\Contracts\Auth\MustVerifyEmail &&! auth()->user()->hasVerifiedEmail())
                    <div>
                        <flux:text class="mt-4">
                            {{ __('Tu email ha sido enviado.') }}

                            <flux:link class="text-sm cursor-pointer" wire:click.prevent="resendVerificationNotification">
                                {{ __('Click aqui para reenviar el email de verificación.') }}
                            </flux:link>
                        </flux:text>

                        @if (session('status') === 'verification-link-sent')
                            <flux:text class="mt-2 font-medium !dark:text-green-400 !text-green-600">
                                {{ __('Nueva verificación ha sido enviada.') }}
                            </flux:text>
                        @endif
                    </div>
                @endif
